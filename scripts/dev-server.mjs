...
const server = createServer(async (req, res) => {
  if (req.method === 'GET' && req.url?.startsWith('/api/refresh-data/status')) {
    json(res, 200, { available: true, running: Boolean(activeRefresh) });
    return;
  }
...
server.listen(PORT, () => {
  console.log(`Port dashboard dev server: http://localhost:${PORT}/`);
  console.log('Typing a missing ticker can auto-run refresh through POST /api/refresh-data.');
});
